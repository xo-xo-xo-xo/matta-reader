...
const MATUGEN_STORAGE_KEY = 'matugen-integration-enabled';
...
export default function MoreSettings({data, actions, fonts}: ExtWrapper & {fonts: string[]}) {
    ...
    const store: {matugenEnabled: boolean; matugenLoaded: boolean} = context.store;
    if (!store.matugenLoaded) {
        store.matugenLoaded = true;
        store.matugenEnabled = false;
        chrome.storage.local.get(MATUGEN_STORAGE_KEY, (result: Record<string, unknown>) => {
            store.matugenEnabled = result[MATUGEN_STORAGE_KEY] as boolean ?? false;
            context.refresh();
        });
    }

    function onMatugenToggle(checked: boolean) {
        store.matugenEnabled = checked;
        chrome.storage.local.set({[MATUGEN_STORAGE_KEY]: checked});
    }
            ...
            {isFirefox ? (
                <Array>
                    ...
                    {__FIREFOX_MV2__ ? (
                        <div class="more-settings__section">
                            <Toggle
                                checked={store.matugenEnabled}
                                labelOn="On"
                                labelOff="Off"
                                onChange={onMatugenToggle}
                            />
                            <p class="more-settings__description">
                                Matugen sync: automatically apply matugen-generated colors
                            </p>
                        </div>
                    ) : null}
                </Array>
